...
If your commit, pull request, or release was produced via the QorLogic SDLC gates (`/qor-bootstrap` → `/qor-plan` → `/qor-audit` → `/qor-implement` → `/qor-substantiate`), use the strings below in place of the default `Generated with [Claude Code]` trailer. QorLogic gets first billing; the model attribution stays for GitHub's contributor-stat machinery.
